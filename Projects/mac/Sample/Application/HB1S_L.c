...
void HBee_LedGetSysTimer(void);
void HBee_LedSendHeatBeat(void);
...
{
    uint8 i;
#define LED0_AlARM_PIN 0   //P0.0
#define LED1_AlARM_PIN 1   //P0.1
#define LED2_AlARM_PIN 2   //P0.2
#define LED3_AlARM_PIN 3   //P0.3
#define LED4_AlARM_PIN 4   //P0.4
#define LED5_AlARM_PIN 5   //P0.5
#define LED6_AlARM_PIN 6   //P0.6
#define LED7_AlARM_PIN 7   //P0.7
    
    for(i=0;i<8;i++)
    {
        IO_FUNC_PORT_PIN(PORT0,i,IO_GIO);
        IO_DIR_PORT_PIN(PORT0,i,IO_OUT);
    }
    P0_0 = 0;
    P0_1 = 0;
    P0_2 = 0;
...
uint16 MSA_ProcessEvent(uint8 taskId, uint16 events)
{
    ...
    if ( events & HBEEAPP_TIME_UPDATEMSG_EVT )
    {
        if(GetTimerflg == 1)
        {
            ...
            {
                HBee_LedGetSysTimer();
                osal_start_timerEx( MSA_TaskId,HBEEAPP_TIME_UPDATEMSG_EVT,3000);
            }
        }
...
void HBee_LedSendHeatBeat(void)
{
    ...
    RF_SendData(RF_L2P_HEART_BEAT_MSG,addr,n,buf);
}
/*********************************************************************
*********************************************************************/
void HBee_LedGetSysTimer(void)
...
/*********************************************************************
*********************************************************************/
void HBee_LedRelpyMsg(RfPc2EndMsg_t *pRfPc2EndMsg)
{
    uint8 *p;
    uint16 addr;
