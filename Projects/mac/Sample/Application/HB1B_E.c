...
void BSP_InitInterrupt(void)
{
    ...
    P0IEN |= 0x07; //P0.012 Enable interrupt Mask
    ...
    P1.7
    */
#define KEY1_PIN 0   //P0.0
#define KEY2_PIN 1   //P0.1
#define KEY3_PIN 2   //P0.2
...
#define M_LED1_PIN 6   //P0.6
#define M_LED2_PIN 5   //P0.5
#define M_LED3_PIN 4   //P0.4
#define M_LED4_PIN 7   //P0.7
    
#define M_LED5_PIN 0   //P1.0
#define M_LED6_PIN 2   //P1.2
#define M_LED7_PIN 3   //P1.3
    
#define M_LED8_PIN 0   //P2.0 //台灯

    IO_FUNC_PORT_PIN( PORT0,M_LED1_PIN, IO_GIO);
    IO_DIR_PORT_PIN(PORT0,M_LED1_PIN,IO_OUT);

    IO_FUNC_PORT_PIN( PORT0,M_LED2_PIN, IO_GIO);
    IO_DIR_PORT_PIN(PORT0,M_LED2_PIN,IO_OUT);

    IO_FUNC_PORT_PIN( PORT0,M_LED3_PIN, IO_GIO);
    IO_DIR_PORT_PIN(PORT0,M_LED3_PIN,IO_OUT);

    IO_FUNC_PORT_PIN( PORT0,M_LED4_PIN, IO_GIO);
    IO_DIR_PORT_PIN(PORT0,M_LED4_PIN,IO_OUT);

    
    IO_FUNC_PORT_PIN( PORT1,M_LED5_PIN, IO_GIO);
    IO_DIR_PORT_PIN(PORT1,M_LED5_PIN,IO_OUT);

    IO_FUNC_PORT_PIN( PORT1,M_LED6_PIN,IO_GIO);
    IO_DIR_PORT_PIN(PORT1,M_LED6_PIN,IO_OUT);

    IO_FUNC_PORT_PIN( PORT1,M_LED7_PIN,IO_GIO);
    IO_DIR_PORT_PIN(PORT1,M_LED7_PIN,IO_OUT);
    
    IO_FUNC_PORT_PIN( PORT2,M_LED8_PIN, IO_GIO);
    IO_DIR_PORT_PIN(PORT2,M_LED8_PIN,IO_OUT);
...
#define READING_LIGHT_PIN P2_0 
    READING_LIGHT_PIN = 0;
 
    
#define BEEP_PIN 7   //P1.7   
    IO_FUNC_PORT_PIN( PORT1,BEEP_PIN, IO_GIO);
    IO_DIR_PORT_PIN(PORT1,BEEP_PIN,IO_OUT); 
#define BEEP_DAT_PIN P1_7
    BEEP_DAT_PIN = 0;
}
/********************************************************************************************************
...
********************************************************************************************************/
#define KEY2_INTERRUPT_FLAG_MASK        0x01//P0.0
#define KEY1_INTERRUPT_FLAG_MASK        0x02//P0.1
#define KEY3_INTERRUPT_FLAG_MASK        0x04//P0.2
HAL_ISR_FUNCTION( MyPort0Isr, P0INT_VECTOR )
{
    ...
    if(DisIntFlg == 0)
    {
        if(((P0&KEY1_INTERRUPT_FLAG_MASK)==0x00)||\
           ((P0&KEY2_INTERRUPT_FLAG_MASK)==0x00)||\
           ((P0&KEY3_INTERRUPT_FLAG_MASK)==0x00))
        {
            DisIntFlg = 1;
            if(((P0&KEY1_INTERRUPT_FLAG_MASK)==0x00)&&\
               ((P0&KEY2_INTERRUPT_FLAG_MASK)==0x00))
            {
                if((P0IFG&KEY1_INTERRUPT_FLAG_MASK)==KEY1_INTERRUPT_FLAG_MASK)
                {
                    BroadcastSend ^= 1;
                ...
                else
                {
                    ...
                    osal_set_event(MSA_TaskId,MSA_DIS_KEY_EVENT);
                    osal_start_timerEx(MSA_TaskId,MSA_END_SET_CHANNLE_EVT,300);
                }
            }
            else
            {
...
#define LED 	P1_0
    ...
    osal_nv_read(WRITER_SET_SEND_ID,0,sizeof(uint8),(uint8*)&SaveBroadcastSend);
    if(SaveBroadcastSend == 0xFF)
    {
        SaveBroadcastSend = 0;  /**< 单播 */
        //SaveBroadcastSend = 1;  /**< 广播 */
        osal_nv_item_init(WRITER_SET_SEND_ID,sizeof(uint8),NULL);
        osal_nv_write(WRITER_SET_SEND_ID,0,sizeof(uint8),(uint8*)&SaveBroadcastSend);
    }
    /**< 进一步判断 */
    if(SaveBroadcastSend > 1)
    {
        SaveBroadcastSend = 0;
    }
    ...
    osal_nv_read(WRITER_SET_CHANNLE_ID,0,sizeof(uint8),(uint8*)&SaveChannle);
    if(SaveChannle == 0xFF)
    {
        SaveChannle = MSA_MAC_CHANNEL;  /**< 信道 */
        osal_nv_item_init(WRITER_SET_CHANNLE_ID,sizeof(uint8),NULL);
...
uint16 MSA_ProcessEvent(uint8 taskId, uint16 events)
{
    ...
    {
        if(KeyValue == 0x00000001)			/**< 呼叫按键标示 */ 
        {
            LedCnt = 0x00;
        }
		else if(KeyValue == 0x0000C001)		/**< 取消按键标示 */
		{
            LedCnt = 0x05;
		}
		else if(KeyValue == 0x80000000)     /**< 心跳标示 */
		{
            KeyValue = 0x00000000;
            return (events ^ MSA_LED_EVENT);
		}
        else if(KeyValue == 0x80000001)     /**< 广播呼叫 */
        {
            LedCnt = 0x02;
        }
        else if(KeyValue == 0x8000C001)     /**< 广播取消 */
        {
            LedCnt = 0x06;
        }
        else
    ...
    if(events&MSA_DIS_KEY_EVENT)
    {
        ...
        P0IEN &= ~0x07;                 //P0.01 DisEnable interrupt Mask
        ...
        if((KeyValue == 0x00000001)||(KeyValue == 0x80000001))
        {
            osal_start_timerEx(MSA_TaskId,MSA_EN_KEY_EVENT,3000);
        }
        else
        {
    ...
    if(events&MSA_EN_KEY_EVENT)
    {
        ...
        P0IEN |= 0x07;                  //P0.01 Enable interrupt Mask
    ...
    if(events&MSA_END_SET_EVT)
    {
        ...
        {
            LedCnt = 0x00;
            if((P0&KEY2_INTERRUPT_FLAG_MASK)==0x00)
            {
