...
/***************Key Interrupt Init****************/
    ...
    P0IEN |= 0x33; //P0.0145 Enable interrupt Mask
...
#define LED2_PIN 5   //P1.5
#define LED3_PIN 6   //P1.6
#define LED4_PIN 7   //P1.7
    
#define LED5_PIN 0   //P2.0
    ...
    IO_FUNC_PORT_PIN( PORT1,LED3_PIN, IO_GIO);
    IO_DIR_PORT_PIN(PORT1,LED3_PIN,IO_OUT);


    IO_FUNC_PORT_PIN( PORT1,LED4_PIN, IO_GIO);
    IO_DIR_PORT_PIN(PORT1,LED4_PIN,IO_OUT);
    
    IO_FUNC_PORT_PIN( PORT2,LED5_PIN, IO_GIO);
    IO_DIR_PORT_PIN(PORT2,LED5_PIN,IO_OUT);
...
HAL_ISR_FUNCTION( MyPort0Isr, P0INT_VECTOR )
{
    ...
    if(DisIntFlg == 0)
    {
        ...
        else if(n==0X23)
        {
            if(KeyNum >=1)
            {
                if(CancelFlg == 0)
                {
                    ...
                    CancelFlg = 1;
                    KeyValue = 0x00000000;
                    osal_start_timerEx(MSA_TaskId,MSA_READ_KEY_EVT,800);
                    goto loop;
                }
                else
                {
        ...
        {
            /**< 按键1,4是否都按下 广播单波，信道设置*/
            if(n==0X03)
            {
                if((P0IFG&KEY4_INTERRUPT_FLAG_MASK) == KEY4_INTERRUPT_FLAG_MASK)
                {
                    Channle++;
                    if(Channle > 17)
                    {
                ...
                else if((P0IFG&KEY1_INTERRUPT_FLAG_MASK) == KEY1_INTERRUPT_FLAG_MASK)
                {
                    BroadcastSend ^= 1;
                    osal_set_event(MSA_TaskId,MSA_DIS_KEY_EVENT);
            ...
            else if(n==0X30)/**< 3键和2键的选择 */
            {
                if((P0IFG&KEY2_INTERRUPT_FLAG_MASK) == KEY2_INTERRUPT_FLAG_MASK)
                {

                    KeyNum ^= 1;
                    osal_set_event(MSA_TaskId,MSA_DIS_KEY_EVENT);
                    osal_start_timerEx(MSA_TaskId,MSA_SET_KEY_NUM_EVT,300);
                }
                else if((P0IFG&KEY3_INTERRUPT_FLAG_MASK) == KEY3_INTERRUPT_FLAG_MASK)
                {
...
#define LED 	P1_0
    ...
    /****************************************************************************************************/
    osal_nv_item_init(WRITER_SET_KEY_NUM_ID,sizeof(uint8),NULL);
    osal_nv_read(WRITER_SET_KEY_NUM_ID,0,sizeof(uint8),(uint8*)&SaveKeyNum);
    if(SaveKeyNum == 0xFF)
    {
        //SaveKeyNum = 0;  /**< 2键 */
        SaveKeyNum = 1;  /**< 4键 */
        osal_nv_item_init(WRITER_SET_KEY_NUM_ID,sizeof(uint8),NULL);
        osal_nv_write(WRITER_SET_KEY_NUM_ID,0,sizeof(uint8),(uint8*)&SaveKeyNum);
    }
    /**< 进一步判断 */
    if(SaveKeyNum > 1)
    {
...
uint16 MSA_ProcessEvent(uint8 taskId, uint16 events)
{
    ...
    {   if(KeyValue == 0x00000000)
        {
          
        }
        else if((KeyValue&0x80000000) == 0x80000000)     /**< 广播包发送 */
        {
            ...
            else
            {
                /**< 广播发送 */
                KeyValue &= 0x7FFFFFFF;
                if((KeyValue <= 0x00000003)&&(KeyValue >= 0x00000001))	        /**< 01~03 */ 
                {
                    LedCnt = 0x01;
                }
                else if((KeyValue <= 0x00008003)&&(KeyValue >= 0x00008001))	    /**< 81~83 */ 
                {
                    LedCnt = 0x04;
                }
                else if((KeyValue == 0x0000C007)||(KeyValue == 0x0000C001))     /**< 全部取消 */ 
                {
                    LedCnt = 0x04;
                }
                else
        ...
        {
            LedCnt++;
            osal_start_timerEx(MSA_TaskId,MSA_LED_EVENT,120);
        }
        else
        {
            LedCnt = 0x00;
    ...
    if(events&MSA_DIS_KEY_EVENT)
    {
        ...
        asm("NOP");
        P0IEN &= ~0x33;                 //P0.0145 DisEnable interrupt Mask
        asm("NOP");
        P2INP |= 0x20;
        asm("NOP");
        P0IFG = 0;
    ...
    if(events&MSA_END_SET_EVT)
    {
        ...
        {
            LedCnt = 0x00;
            BSP_ClearBroadcastLed(0);
            if((P0&KEY4_INTERRUPT_FLAG_MASK)==0x00)
            {
    ...
    if(events&MSA_SET_KEY_NUM_EVT)
    {
        ...
        if(KeyNum == 1)
        {
            BSP_SetKeyNumLed(LedCnt);
        }
        else
        {
            BSP_ClearKeyNumLed(LedCnt);
        }
      
        return (events^MSA_SET_KEY_NUM_EVT);
    }
